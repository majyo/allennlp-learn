...
def restore_and_evaluate() -> Dict[str, Any]:
    allennlp_args = argparse.Namespace()
    allennlp_args.file_friendly_logging = False
...
def construct_params_for_predict() -> argparse.Namespace:
    allennlp_args = argparse.Namespace()
    allennlp_args.archive_file = "result/model.tar.gz"
    allennlp_args.input_file = "data/movie_review/test.jsonl"
    allennlp_args.output_file = "predct"
...
def restore_predictor(args: argparse.Namespace) -> Predictor:
...
def predict(args: argparse.Namespace, predictor: Predictor):
...
def predict_json(input_json: Dict[str, Any], predictor:Predictor):
    input_json = input_json if input_json else {"sentence": "A good movie!"}
    output = predictor.predict_json(input_json)
    print(output)


def run_with_console_out():
    train()


def run_with_file_out():
...
if __name__ == "__main__":
    args = construct_params_for_predict()
    predictor = restore_predictor(args)
